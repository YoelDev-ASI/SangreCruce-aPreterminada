...
export class EscenaMapa {
    ...
    constructor(anchoLogico, altoLogico, motor) {
        ...
        this.imagenFondo.src = 'assets/imagenes/MapaSantaCruz.png';
        ...
        // Lista de misiones de la base de datos (filtrando dinámicamente el nodo de Santa Cruz)
        const completadoNivel1 = this.nivelesCompletados.includes('revolucion_1');
        this.misiones = datosBatallas.filter(m => {
            if (m.id === 'revolucion_1') {
                return !completadoNivel1;
            }
            if (m.id === 'liberacion_final') {
                return completadoNivel1;
            }
            return true;
        });
        ...
        sonidos.playMusica('assets/audio/musica/SonidoEscenaPrincipal.mp3');
    }
    ...
    draw(ctx) {
        ...
        if (this.imagenFondo && this.imagenFondo.complete && this.imagenFondo.naturalWidth !== 0) {
            ctx.drawImage(this.imagenFondo, 0, 0, this.anchoLogico, this.altoLogico);
        } else {
            // Fondo de pergamino de respaldo mientras carga
        ...
        for (const m of this.misiones) {
            ...
            if (!bloqueado) {
                const pulsoRadius = 35 + Math.sin(Date.now() * 0.007) * 4;
