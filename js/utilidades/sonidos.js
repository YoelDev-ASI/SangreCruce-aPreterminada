...
export class SonidoManager {
    constructor() {
        ...
        this._removerListenersInteraccion = null;
    }
    ...
    playMusica(src) {
        ...
        // Detener música previa y limpiar listeners de interacción anteriores
        this.detenerMusica();
        ...
        try {
            ...
            this.musicaActual.play().catch(err => {
                ...
                const iniciarMusica = () => {
                    if (this.musicaActual && !this.muted) {
                        this.musicaActual.play()
                            .then(() => {
                                if (this._removerListenersInteraccion) {
                                    this._removerListenersInteraccion();
                                }
                            })
                            .catch(e => {
                                console.warn("Todavía no se pudo reproducir el audio al interactuar:", e);
                            });
                    } else {
                        if (this._removerListenersInteraccion) {
                            this._removerListenersInteraccion();
                ...
                const eventos = ['click', 'pointerdown', 'keydown', 'touchstart'];
                
                this._removerListenersInteraccion = () => {
                    eventos.forEach(evt => {
                        document.removeEventListener(evt, iniciarMusica);
                    });
                    this._removerListenersInteraccion = null;
                };

                eventos.forEach(evt => {
                    document.addEventListener(evt, iniciarMusica);
                });
            });
        } catch (error) {
