...
(function () {
    ...
    function esOrientacionVertical() {
        ...
        // Fallback robusto comparando dimensiones de la ventana
    ...
    function intentarEntrarPantallaCompleta() {
        ...
        if (requestFS) {
            // Intentamos la activación automática
            requestFS.call(docEl)
                ...
                .catch((err) => {
                    ...
                    // Si se bloquea por falta de gesto previo, registramos el fallback
    ...
    function registrarGestoPantallaCompleta() {
        const activarFS = () => {
            // Solo solicitar si seguimos en modo horizontal y el overlay no está activo
            if (!esOrientacionVertical()) {
                ...
                const requestFS = docEl.requestFullscreen || 
                                  ...
                                  docEl.msRequestFullscreen;
                if (requestFS) {
                    requestFS.call(docEl).catch(err => console.log("[Orientación] Falla al entrar en pantalla completa por gesto:", err));
                }
            }
