...
export function renderMinero(ctx, minero) {
    ...
    const colorCuerpo = esPatriota ? '#00796b' : '#b22222';
    ...
    if (esPatriota) {
        ...
        const callbackArma = (c) => {
            // El pico se mueve rítmicamente al minar
            let anguloPico = minero.anguloPico;
            dibujarPico(c, 0, 0, anguloPico, 5.55, minero.bando);
        };
        dibujarEsqueletoPatriota(ctx, minero, colorCuerpo, callbackArma);
    } else {
        // Dibujo articulado con esqueleto realista
        const callbackArma = (c) => {
            // El pico se mueve rítmicamente al minar
            let anguloPico = minero.anguloPico;
            dibujarPico(c, 0, 0, anguloPico, 5.55, 'patriota');
        };
        dibujarEsqueletoRealista(ctx, minero, colorCuerpo, callbackArma);
    }
